...
- **Do NOT build an `AWS::RDS::DBSubnetGroup` from the unfiltered private-subnet export** — prune it to the AZs that offer *every* `DBInstanceClass` in the group via `RdsClientWrapper.getAvailableSubnetsForDBInstanceClasses` (RDS `DescribeOrderableDBInstanceOptions`, **not** the EC2 `DescribeInstanceTypeOfferings` used for `ec2`/OpenSearch — EC2 knows nothing about `db.*` classes). If a Multi-AZ standby lands in a zone that doesn't offer the class, RDS accepts the `ModifyDBInstance`, abandons the conversion with **no failure event**, and returns the instance to `available`, so CloudFormation reports success on a database that is silently single-AZ — and the drift self-perpetuates because the template value never changes again. Post-deploy, assert actual vs. expected `MultiAZ` (`RdsClientWrapper.validateMultiAZ`) rather than trusting stack completion (PLFM-9965).
